...
public class AuthActuatorSecurityConfig {
    ...
    /**
     * 创建仅供 Actuator Basic Auth 使用的本地用户。
     *
     * <p>认证服务已有 BCrypt 密码编码器，因此需要在启动时将 Nacos 中的
     * 本地学习密码编码后再交给 Spring Security 校验。</p>
     *
     * @param securityProperties Spring Security 用户属性
     * @param passwordEncoder BCrypt 密码编码器
     * @return Actuator 用户详情服务
     */
    @Bean
    public UserDetailsService actuatorUserDetailsService(SecurityProperties securityProperties, PasswordEncoder passwordEncoder) {
        String username = securityProperties.getUser().getName();
        String rawPassword = securityProperties.getUser().getPassword();

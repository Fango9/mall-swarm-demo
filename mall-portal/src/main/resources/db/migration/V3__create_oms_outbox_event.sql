...
(
    ...
    aggregate_type VARCHAR(32)    NOT NULL COMMENT '聚合类型，例如 ORDER',
    aggregate_id  BIGINT UNSIGNED NOT NULL COMMENT '聚合主键，例如订单 ID',
    event_type    VARCHAR(64)     NOT NULL COMMENT '事件类型，例如 ORDER_CREATED',
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_oms_outbox_event_event_id (event_id),
    UNIQUE KEY uk_oms_outbox_event_aggregate_event
        (aggregate_type, aggregate_id, event_type),
    KEY idx_oms_outbox_event_status_retry (status, next_retry_at)
) ENGINE = InnoDB
